...
const CLIENT_COLORS = {
  ...
  SENT:               { bg: '#e8f4ff', color: '#1a7fd4', dot: '#1a7fd4', border: '#a8d4f5', label: 'New' },
...
function StatusBadge({ status, external }) {
  const s = (external ? CLIENT_COLORS : REVIEW_COLORS)[status] || { bg: '#f1f5f9', color: '#475569', dot: '#94a3b8', border: '#e2e8f0', label: status }
  return (
    <span style={{
...
function ContactCell({ phone, name }) {
  ...
  return (
    ...
    >
      <span style={{ color: '#2563eb', cursor: 'pointer', fontSize: 13, fontWeight: 500 }}>
        {phone || '—'}
      </span>
      {hover && phone && (
        <div style={{
...
// ─── Stat card icons ──────────────────────────────────────────────────────────
const STAT_ICONS = {
  'Team Approved Quotes': 'M9 11l3 3L22 4M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11',
  'Quoted':               'M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6',
  'Under Negotiation':    'M17 1l4 4-4 4M3 11V9a4 4 0 0 1 4-4h14M7 23l-4-4 4-4M21 13v2a4 4 0 0 1-4 4H3',
  'Quotes Accepted':      'M9 12l2 2 4-4m6 2a9 9 0 1 1-18 0 9 9 0 0 1 18 0z',
...
export default function ManagerQuotations() {
  ...
  const fetchData = useCallback(() => {
    setLoading(true)
    Promise.all([api.get('/quotations/'), api.get('/quotations/dashboard_stats/')])
      .then(([qRes, sRes]) => {
  ...
  const rows = filtered.map(q => {
      ...
      if (isExternal) row.push(q.target_submission_date || '')
      row.push(q.assigned_to_name || '', q.grand_total ? `₹${Number(q.grand_total).toLocaleString('en-IN')}` : '', q.quote_days != null ? `${q.quote_days} Days` : '', toPriorityKey(q.priority), q.phone_mobile || '')
      if (isExternal) row.push(loc)
      return row.map(v => v || '')
    })
  ...
  const statCards = [
    ...
    { label: 'Quoted',               value: stats.under_review ?? 0, highlight: false },
    ...
    { label: 'Quotes Rejected',      value: stats.rejected     ?? 0, highlight: true  },
  ]
    ...
    <div style={{ fontFamily: FONT, background: '#f8fafc', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', zIndex: 2 }}>
          ...
          <button
            onClick={() => navigate('/manager/enquiries/')}
            style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <button
                onClick={() => navigate('/manager/quotation-data')}
        ...
        <div style={{ overflowX: 'auto', borderRadius: 10, border: `1px solid ${BORDER}` }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: isExternal ? 1200 : 900 }}>
            <thead>
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={HEADERS.length} style={{ padding: 48, textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>
