...
const ic = {
  ...
  eye:       'M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8zM12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8z',
  calendar:  'M19 4h-1V2h-2v2H8V2H6v2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2zM8 8h8M3 10h18',
}
...
// Compute totals directly from items array
...
function LineItemsTable({ items, onRemove, onEdit }) {
  const cols = ['Job Code','Cust. Part No','Part No.','Name / Description','HSN Code','Qty','Unit','Unit Price','Tax %','Tax Amt','Total (incl. tax)','Actions']
  return (
...
// ── MANAGER SIDEBAR (only shown for manager role) ──
const PRIORITY_ORDER = ['LOW', 'MEDIUM', 'HIGH']
const PRIORITY_COLORS = { LOW: '#22c55e', MEDIUM: '#f59e0b', HIGH: '#ef4444' }
const PRIORITY_LABELS = { LOW: 'Low', MEDIUM: 'Medium', HIGH: 'High' }

function FlagIcon({ active, color, size = 20 }) {
...
function ManagerSidebar({ enquiryId }) {
  ...
  const resolveEmpId = emp => emp?.user?.id ?? emp?.id ?? null
  const resolveEmpName = emp => {
    if (!emp) return null
  ...
  const handleSetPriority = async (priority) => {
    ...
    setSavingP(true)
    try {
      const res = await api.patch(`/enquiries/${enquiryId}/`, { priority })
      setEnquiry(res.data)
      showToast(`Priority set to ${PRIORITY_LABELS[priority]}`)
    } catch { alert('Failed to set priority.') }
    finally { setSavingP(false) }
  }

  const handleAssign = async () => {
    if (!assignTo || !enquiryId) return
    setSavingA(true)
    try {
      await api.post(`/enquiries/${enquiryId}/assign/`, { assigned_to: assignTo })
  ...
  if (!enquiryId) return (
    <div style={{ background:'#F9FAFB', borderRadius:10, border:'1px solid #e5e7eb', padding:'18px 20px' }}>
      <div style={{ fontSize:13, color:'#9CA3AF' }}>No linked enquiry found.</div>
    </div>
  )

  if (!enquiry) return (
    <div style={{ background:'#fff', borderRadius:10, border:'1px solid #e5e7eb', padding:'18px 20px', display:'flex', alignItems:'center', justifyContent:'center' }}>
  ...
  const daysSince = enquiry.last_activity_at
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      {toast && (
        <div style={{ background:PRIMARY, color:'#fff', borderRadius:8, padding:'10px 16px', fontSize:13.5, textAlign:'center' }}>{toast}</div>
      )}

      {/* Activity Days */}
      <div style={{ background:'#fff', borderRadius:10, border:'1px solid #e5e7eb', padding:'18px 20px' }}>
        <div style={{ fontSize:11.5, fontWeight:700, color:'#6B7280', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:10 }}>Activity</div>
        <div style={{ fontSize:36, fontWeight:700, lineHeight:1, color:daysSince > 7 ? '#ef4444' : daysSince > 3 ? '#f59e0b' : PRIMARY }}>
          {daysSince !== null ? daysSince : '—'}
          <span style={{ fontSize:15, fontWeight:400, color:'#9CA3AF', marginLeft:5 }}>Days</span>
        </div>
      ...
      {/* Set Priority */}
      <div style={{ background:'#fff', borderRadius:10, border:'1px solid #e5e7eb', padding:'18px 20px' }}>
        <div style={{ fontSize:11.5, fontWeight:700, color:'#6B7280', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:14 }}>Set Priority</div>
        <div style={{ display:'flex', gap:14, alignItems:'center' }}>
          {PRIORITY_ORDER.map(p => {
            const isActive = enquiry.priority === p
            return (
              <button key={p} onClick={() => !savingP && handleSetPriority(p)} disabled={savingP} title={PRIORITY_LABELS[p]}
                style={{ background:'none', border:'none', cursor:savingP?'wait':'pointer', padding:3, borderRadius:5, opacity:isActive?1:0.3, transform:isActive?'scale(1.25)':'scale(1)', transition:'all .15s' }}>
                <FlagIcon active color={PRIORITY_COLORS[p]} size={22} />
              </button>
            )
          })}
        </div>
        <div style={{ marginTop:10, fontSize:13.5, color:'#4B5563' }}>
      ...
      {/* Assigned To */}
      <div style={{ background:'#fff', borderRadius:10, border:'1px solid #e5e7eb', padding:'18px 20px' }}>
        <div style={{ fontSize:11.5, fontWeight:700, color:'#6B7280', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:14 }}>Assigned To</div>
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          <select value={assignTo} onChange={e => setAssignTo(e.target.value)}
            style={{ width:'100%', padding:'10px 12px', border:'1.5px solid #e5e7eb', borderRadius:8, fontSize:14, fontFamily:FONT, color:'#374151', outline:'none', background:'#fff', cursor:'pointer' }}>
            <option value="">— Unassigned —</option>
            {employees.map(emp => {
              const eid = String(resolveEmpId(emp))
              const name = resolveEmpName(emp) || eid
              return <option key={eid} value={eid}>{name}</option>
            })}
          </select>
          <button onClick={handleAssign}
            disabled={savingA || !assignTo || assignTo === String(enquiry.assigned_to || '')}
            style={{ padding:'10px', border:'none', borderRadius:8, fontSize:14, fontWeight:600, cursor:'pointer', transition:'all .15s',
              background:(savingA || !assignTo || assignTo === String(enquiry.assigned_to || '')) ? '#F3F4F6' : PRIMARY,
              color:(savingA || !assignTo || assignTo === String(enquiry.assigned_to || '')) ? '#9CA3AF' : '#fff',
            }}>
            {savingA ? 'Assigning...' : 'Confirm Assignment'}
          </button>
        </div>
      </div>
      ...
      <div style={{ background:'#F3F4F6', borderRadius:10, border:'1px solid #e5e7eb', padding:'16px 18px' }}>
        <div style={{ fontSize:11.5, fontWeight:700, color:'#4B5563', marginBottom:12, textTransform:'uppercase', letterSpacing:'0.5px' }}>Quick Info</div>
        <div style={{ display:'flex', flexDirection:'column', gap:9 }}>
          {[
            ['Enquiry No', enquiry.enquiry_number],
            ['Status', enquiry.status],
            ['Priority', PRIORITY_LABELS[enquiry.priority]||'—'],
            ['Due Date', enquiry.due_date || '—'],
          ].map(([label, value]) => (
            <div key={label} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:8 }}>
              <span style={{ fontSize:13, color:'#6B7280' }}>{label}</span>
              <span style={{ fontSize:13, fontWeight:600, color:PRIMARY }}>{value || '—'}</span>
            </div>
          ))}
        </div>
      </div>
...
/* ════════════════════════════════════════════════════════════
   MAIN COMPONENT - Unified for both Employee and Manager
════════════════════════════════════════════════════════════ */
export default function OADetail({ basePath = '/employee/order-acknowledgements', role = 'employee' }) {
  const { id } = useParams()
  ...
  const openEditItem = i => setShowProductModal(true)
  ...
  const enquiryId = oa?.enquiry || null
  ...
  return (
    <div style={{ fontFamily:FONT, width:'100%', minHeight:'100vh', background:'#f8f9fb', padding:'0 0 48px' }}>
      ...
      {/* MAIN CONTENT - Two column layout for manager, single column for employee */}
      <div style={{ padding:'20px 28px' }}>
        <div style={{ display: isManager ? 'grid' : 'block', gridTemplateColumns: isManager ? '1fr 290px' : '1fr', gap: isManager ? 22 : 0, alignItems:'start' }}>
          
          {/* LEFT COLUMN - Main OA Content (same for both roles) */}
          <div>
            {/* SECTION 1: OA INFO */}
            <div style={{ background:'#fff', borderRadius:10, padding:'20px 24px', border:'1px solid #e8edf2', boxShadow:'0 1px 3px rgba(0,0,0,0.04)', marginBottom:16 }}>
          ...
          {/* RIGHT COLUMN - Manager Sidebar (only shown for manager role) */}
          {isManager && (
            <div style={{ position:'sticky', top:24 }}>
              <ManagerSidebar enquiryId={enquiryId} />
            </div>
          )}
        </div>
      </div>
